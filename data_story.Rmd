...
## Scraping wikipedia
Lets begin by reading the two tables of the *100 meters per age category* section. I wrote a funtion using the rvest library to read a HTML table given the corresponding XPATH from the wiki url once. The table is stored locally as a csv file and loaded loacally if available. Then I call the function with the arguments for the tables of interest.
...
```
The linear regression (blue) shows nicely what one expects: While age has a big impact on the performance of young athletes, the impact is lost over time. It seems that young athletes improve their performance in close relation to getting older, while from maybe 16 onwards the age factor appears to lose it's weight.

## Age and adult athletes best performance
This promped the follow up question: At what age did the all-time fastest athletes run their record times?
...
```
Now lets collect the birthdays from their personal page. I take advantage of the typical infobox that's located on the top right of most wiki pages. However six of the entries aren't caught in the pattern (for various reasons) - I insert them in a postprocessing step.
...
```
Bringind the two tables together we can analyse the data. The overall mean age at peak performance of the top 25 women and men amounts to 25.8 years. The athletes gender appears to have no significant influence on the age the athletes set their records.
...
```
```{r box plotts, echo=FALE, message=FALSE}
